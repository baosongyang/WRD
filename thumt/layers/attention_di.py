...
def multiplicative_attention(queries, keys, values, bias, gernal_heads, di_heads, keep_prob=None,name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        k_len = tf.shape(logits)[3]
        gaussian = []
        if gernal_heads:
            gaussian.append(tf.zeros([1,gernal_heads,k_len,k_len]))
        if di_heads:
            lower_triangle = tf.matrix_band_part(tf.ones([k_len, k_len]), -1, 0)
            ret = (-1e9) * (1.0 - lower_triangle)
            forward=tf.reshape(ret, [1, 1, k_len, k_len])
            backward = tf.transpose(forward,[0,1,3,2])
            gaussian.append(tf.tile(tf.concat([forward,backward],axis=1),[1,di_heads,1,1]))
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        ...
                        state=None, dtype=None, scope=None, di_heads=4):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, num_heads-2*di_heads, di_heads, keep_prob)
