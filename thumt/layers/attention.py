...
def multiplicative_attention(queries, keys, values, bias, head_dim, keep_prob=None,
                             name=None):
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    if local_heads+long_heads > num_heads:
        raise ValueError("totoal heads number (%d) larger than num_heads (%d) must be divisible by the number of " % (local_heads+long_heads, num_heads))						 
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, key_depth_per_head, keep_prob)
