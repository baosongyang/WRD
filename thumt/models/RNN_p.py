...
from thumt.models.rnnsearch import _gru_encoder
...
def transformer_encoder(inputs, sequence_length, params, dtype=None, scope=None):
	...
	x= inputs	
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                inputs_fw = _layer_process(x, params.layer_preprocess)
			...
			inputs_fw,
                ...
                with tf.variable_scope("backward"):
                    y_bw,_ = _gru_encoder(
			cell_bw,
			tf.reverse_sequence(inputs_fw,sequence_length,batch_axis=0,seq_axis=1),
                        sequence_length,
                        None,
			dtype=dtype
		    )
		    y_bw = tf.reverse_sequence(y_bw, sequence_length,batch_axis=0, seq_axis=1)
		y = layers.nn.linear(tf.concat([y_fw,y_bw],axis=2),params.hidden_size,True,True,scope="concat_transform")
...
def encoding_graph(features, mode, params):
    ...
    encoder_output = transformer_encoder(encoder_input, src_len, params)
