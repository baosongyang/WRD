...
def get_inference_input(filename, params,ifrepeat=True,ifshuffle=True):
    #inputs = [[],[]]
    #inputs2 = [[],[]]
    #with tf.gfile.Open(filename[0]) as fd:
    #    inputs2[0] = [line.strip() for line in fd]
    #with tf.gfile.Open(filename[1]) as fd:
    #    inputs2[1] = [line.strip() for line in fd]
    #t1 = np.arange(len(inputs2[0]))
    #import random
    #if 'tr' in filename[0]:
    #    epoch = 2
    #else:
    ...
    #    random.shuffle(t1)
    #    for i, item in enumerate(inputs2[0]):
    #        inputs[0].append(inputs2[0][t1[i]])
    #        inputs[1].append(inputs2[1][t1[i]])
    

    with tf.device("/cpu:0"):
        src_dataset = tf.data.TextLineDataset(filename[0])
        tgt_dataset = tf.data.TextLineDataset(filename[1])
        dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset))
        #dataset = tf.data.Dataset.zip(tuple([tf.data.Dataset.from_tensor_slices(filename[0]),tf.data.Dataset.from_tensor_slices(filename[1])]))
        #(    tf.constant(inputs[0])
        #   ,tf.constant(inputs[1]))
        #)
        if ifshuffle:
            dataset=dataset.shuffle(7000000)
        if ifrepeat:
            dataset = dataset.repeat()
        else:
            dataset=dataset.repeat()
        ...
        # Split string
        dataset = dataset.map(lambda x,y: (tf.string_split([x]).values, tf.string_to_number(tf.string_split([y]).values, out_type = tf.int32)),
                              num_parallel_calls=params.num_threads)

        # Append <eos>
        dataset = dataset.map(
            lambda x, y: (tf.concat([x, [tf.constant(params.eos)]], axis=0), y),
            num_parallel_calls=params.num_threads
        )

        # Convert tuple to dictionary
        dataset = dataset.map(
                lambda x, y: {"source": x, "source_length": tf.shape(x)[0], "target": y},
            num_parallel_calls=params.num_threads
        )

        dataset = dataset.padded_batch(
            params.decode_batch_size * len(params.device_list),
            {"source": [tf.Dimension(None)], "source_length": [], "target": [2]},
            {"source": params.pad, "source_length": 0, "target": 0}
        )
